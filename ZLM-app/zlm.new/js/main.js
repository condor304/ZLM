...
if (!loaded) $( document ).bind('pageinit', onDeviceReady);
...
function onDeviceReady() {
	try
	{
		...
		if (lang==="ro")
		{
			_("span", "Choose your gender", "Alege sexul");
			_("span", "Male", "Masculin");
			_("span", "Female", "Feminin");
			_("td", "Choose your weight", "Alege greutatea");
			_("td", "Choose your height", "Alege inaltimea");
			_("a", "Next step", "Pasul urmator");
			_("span", "Your profile is ready. You can change your data whenever you want.", "Ti-ai setat profilul. Iti poti schimba datele oricand doresti.");
			_("span", "Start by adding a drink and create the combination you drank, to find out when your blood alcohol level will reach ZERO.", "Incepe prin a adauga o bautura si creeaza combinatia pe care ai consumat-o pentru a afla cand vei avea ZERO LA MIE alcoolemie in sange.");
			_("span", 'Press the button above "Add a drink" and start the process.', "Apasa butonul \"Adauga o bautura\" si poti incepe.");
			_("span", "Add a drink", "Adauga o bautura");

			_("td", "Beer", "Bere");
			_("td", "Wine", "Vin");
			_("td", "Spirits", "Spirtoase");

			_("td.label", "Quantity", "Cantitate");
			_("a", "Add the drink", "Adauga bautura");
			_("span", "Add another drink", "Adauga inca o bautura");

			_("a", "Set your time", "Seteaza timpul");
			_("a", "Calculate time", "Calculeaza timpul");

			_("td.label", "I started at", "Am inceput la");
			_("td.label", "I finished at", "Am incheiat la");
			...
			_("h3", "Add 90 minutes to the end result if you've consumed any food.", "Adauga 90 minute la rezultatul primit daca ai consumat alimente.");
			...
			_("div.discreg", "Valid in case of constant alcohol consumption only.", "Valabil numai in cazul unui consum constant de alcool.");
		...
		if (!zero_la_mie)
		{
			...
			zero_la_mie.istoric = [];
			zero_la_mie.alcool_pur = 0;
			zero_la_mie.unitate_greutate = "kg";
			zero_la_mie.unitate_inaltime = "cm";
		}
		else
		{
			zero_la_mie.bauturi = [];
		...
		{
			$('.tab.bio img:not(.selected)').attr('src', $('.tab.bio img:not(.selected)').attr('src').replace('female', 'male'));
			$('#male img').attr('src', $('#male img').attr('src').replace('0', '1'));
		}

		if (zero_la_mie.sex==="F")
		{		
			$('.tab.bio img:not(.selected)').attr('src', $('.tab.bio img:not(.selected)').attr('src').replace('male', 'female'));
			$('#female img').attr('src', $('#female img').attr('src').replace('0', '1'));
		}
		...
		if (zero_la_mie.unitate_inaltime)
		{
			$("#height_unit option[value="+zero_la_mie.unitate_inaltime+"]").attr('selected','selected');
		}

		if (zero_la_mie.unitate_greutate)
		{
			$("#weight_unit option[value="+zero_la_mie.unitate_greutate+"]").attr('selected','selected');
		}
		...
		$('#add_some_drinks').unbind().live(click_event, function(event){
			...
			zero_la_mie.alcool_pur = 0;
			localStorage.setObj("zero_la_mie", zero_la_mie);
			zero_la_mie.inaltime = parseFloat($('#height').val());
			zero_la_mie.greutate = parseFloat($('#weight').val());
			if (!zero_la_mie.sex)
			{
				if (typeof event !== "undefined") { event.preventDefault(); event.stopImmediatePropagation(); }
				
				if (lang==="ro")
					navigator.notification.alert("Trebuie sa specifici sexul.", alertDismissed);
				else 
					navigator.notification.alert("You must choose the sex.", alertDismissed);
			...
			{
				if (lang==="ro")
					navigator.notification.alert("Trebuie sa specifici greutatea.", alertDismissed);
				else 
					navigator.notification.alert("You must choose the weight.", alertDismissed);
				return false; if (typeof event !== "undefined") { event.preventDefault(); event.stopImmediatePropagation(); }
			}
			...
			{
				if (lang==="ro")
					navigator.notification.alert("Trebuie sa specifici intaltimea.", alertDismissed);
				else 
					navigator.notification.alert("You must choose the height.", alertDismissed);
				return false; if (typeof event !== "undefined") { event.preventDefault(); event.stopImmediatePropagation(); }
			}
		...
		// Step 3
		$('.step3 #next_step').unbind().live(click_event, function(event){
			if (typeof cls === "undefined")
			{
				navigator.notification.alert("You must select a drink type", alertDismissed);
				return false; if (typeof event !== "undefined") { event.preventDefault(); event.stopImmediatePropagation(); }
			}
			...
			{
				navigator.notification.alert("You must complete the alcohol concentration", alertDismissed);
				return false; if (typeof event !== "undefined") { event.preventDefault(); event.stopImmediatePropagation(); }
			}
			...
			{
				if (lang==="ro")
					navigator.notification.alert("Trebuie sa specifici cantitatea.", alertDismissed);
				else 
					navigator.notification.alert("You must choose the quantity.", alertDismissed);
				return false; if (typeof event !== "undefined") { event.preventDefault(); event.stopImmediatePropagation(); }
			}
			...
			if ($('.step5 .list .addbutton table').length===1 && $('.step5 .list .addbutton table .icon').html()==="")
			{
				switch (zero_la_mie.tip_bautura)
				{
					case "beer":
					case "bere":
						icon_path = "img/icon.beer.list.png";
					break;

					case "wine":
					case "vin":
						icon_path = "img/icon.wine.list.png";
					break;

					case "spirits":
					case "spirtoase":
						icon_path = "img/icon.spirits.list.png";
					break;
				}
		...
		$('#calculate_time').unbind().live(click_event, function(event){
			...
			if(zero_la_mie.ora_inceput+':'+zero_la_mie.minute_inceput === ':') {
				if (lang==="ro")
					navigator.notification.alert('Trebuie sa specifici cand ai inceput sa bei.', alertDismissed);
				else
					navigator.notification.alert('You must specify when you started drinking.', alertDismissed);
				return false; if (typeof event !== "undefined") { event.preventDefault(); event.stopImmediatePropagation(); }
			}
			...
			if (zero_la_mie.ora_sfarsit+':'+zero_la_mie.minute_sfarsit === ':') {
				if (lang==="ro")
					navigator.notification.alert('Trebuie sa specifici cand ai terminat sa bei.', alertDismissed);
				else
					navigator.notification.alert('You must specify when you finished drinking.', alertDismissed);
				return false; if (typeof event !== "undefined") { event.preventDefault(); event.stopImmediatePropagation(); }
			}
			...
			if (zero_la_mie.ora_sfarsit && zero_la_mie.minute_sfarsit!=='' && zero_la_mie.ora_inceput && zero_la_mie.minute_inceput!=='')
			{
				if (strtotime(date('Y-m-d')+' '+zero_la_mie.ora_sfarsit+':'+zero_la_mie.minute_sfarsit) <= strtotime(date('Y-m-d')+' '+zero_la_mie.ora_inceput+':'+zero_la_mie.minute_inceput))
				{
					...
					{
						if (lang==="ro")
							navigator.notification.alert("Ora de inceput trebuie sa fie inainte de ora de sfarsit.", alertDismissed);
						else
							navigator.notification.alert('The hour when you finished drinking must be after the start hour.', alertDismissed);
						return false; if (typeof event !== "undefined") { event.preventDefault(); event.stopImmediatePropagation(); }
					}
					else
					{
			...
			if (zero_la_mie.unitate_inaltime=="ft")
			{
				var inaltime = parseInt(zero_la_mie.inaltime / 0.0328084);
			}
			else var inaltime = zero_la_mie.inaltime;

			if (zero_la_mie.unitate_greutate=="lb")
			{
				var greutate = parseInt(zero_la_mie.greutate / 2.20462);
			}
			else var greutate = zero_la_mie.greutate;

			var r = getWidmarkFactor(zero_la_mie.sex, greutate, inaltime);
			...
			zero_la_mie.concentratia = zero_la_mie.alcool_pur / ( greutate * r );
		...
		*/
		...
		$('#history_list a').unbind().live("click", function(event){
			...
			zero_la_mie.bauturi = [];
			zero_la_mie.alcool_pur = 0;
			zero_la_mie.alcoolemia_maxima = 0;
		...
		$('#weight_unit').unbind().live("change", function(event){
			zero_la_mie.unitate_greutate = $(this).val();
		});

		$('#height_unit').unbind().live("change", function(event){
			zero_la_mie.unitate_inaltime = $(this).val();
		});
		...
		$('#iconsmall').unbind().live(click_event, function(event){
			...
			{
				$('img', this).attr('src', 'img/icon.bottle.small.png');
				$('.unitate').html('330');
			}
			else if ($('img', this).attr('src').indexOf('icon.bottle.small.png')!==-1)
			{
				$('img', this).attr('src', 'img/icon.beer.small.png');
				$('.unitate').html('500');
			}
			return false; if (typeof event !== "undefined") { event.preventDefault(); event.stopImmediatePropagation(); }
		});

		$('.drink:not(.inputs)').unbind().live(click_event, function(event){
			...
			{
				navigator.notification.alert("You must specify an alcohol concentration.", alertDismissed);
				return false; if (typeof event !== "undefined") { event.preventDefault(); event.stopImmediatePropagation(); }
			}
			...
			$('span.alcvol').html(concentratie_alcool);
			$('#icon').attr('src', 'img/icon.'+cls+'.small.png');
			if (lang==="ro")
			{
	...
	{
		lastError = printStackTrace().join('\n\n');
		console.log(e);
	}
}
...
function refreshKnob() {
	...
	$('#percent').val(number).trigger("change");
}
...
function computeHistory() {
	...
	for ( index in zero_la_mie.istoric )
	{
		...
		if (summary.beer)
		{
			line = line + '<img src="img/history.beer.png"><span> '+round(summary.beer/1000, 2)+' l </span> ';
		}
		if ((summary.wine && summary.beer) || (summary.spirits && summary.beer))
		{
			line = line + '<img src="img/add.png" align="middle" />' ;
		}

		if (summary.wine)
		{
			line = line + '<img src="img/history.wine.png"><span> '+round(summary.wine/1000, 2)+' l </span> ';
		}
		...
		if (summary.spirits)
		{
			line = line + '<img src="img/history.spirits.png"><span> '+round(summary.spirits/1000, 2)+' l </span> ';
		}
		line = line +'</a>';
